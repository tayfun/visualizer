...
class AssertionError_(AssertionError): pass
...
class NodeToJS(): 
    ...
    def convert(self, n, i="", c={}, handledattrs=[]):
        ...
        try:
            ...
            if callable(getattr(self, convert_type, None)):
                return self.dump_code(n, i, c, convert_type)
    ...
    def dump_code(self, n, i, c, convert_type):
        if getattr(n, "parenthesized", False): t = "(%s)"
        else: t = "%s"
        return t % (getattr(self, convert_type)(n,i,c))
    ...
    def convert_script(self,n,i,c,optattrs=[]):
        ...
        if len(n)>0:
            return "{" + i + \
                        (i.join((
                            "%s%s" % (self.o(x,i,c), self.semicolon) for x in n))) \
                        + "s}"
        else:
            return "{}"
    ...
    def convert_identifier(self, n, i, c):
        self.check(n,optattrs=["initializer","name","readOnly"])
#            if getattr(n,"readOnly",False): assert hasattr(n,"initializer")
        if hasattr(n,"name"): assert n.name == n.value, n.name + " is not equal to " + n.value
    ...
    def convert_if(self, n, i, c):
        ...
        #useParens = self.paren_free_mode and "%s" or "(%s)"
        #braces = self.paren_free_mode and
        if n.elsePart:
    ...
    def convert_string(self, n, i, c):
        ...
        return n.quotes + repr(n.value)[1:-1] + n.quotes
